...
class TrainClient():
    ...
    def init_model(self):
        ...
        from config import Config as Bert_Config
        self.bert_model = BERT4IntentDetection(Bert_Config)

        if self.configs.is_training:
            ckpt = torch.load(self.configs.intent_model, map_location=None)
            self.bert_model.load_state_dict(ckpt)
    ...
    def run(self):
        ...
        if self.configs.is_training:
            ...
            self.model.init_projection(self.bert_model)
            self.model.to(device)
            print(self.model)

            model_path = self.configs.test_model_path
    ...
    def run_train(self):
        ...
        model_path = self.configs.ftp_model_path

        if model_path != '' or (not self.configs.is_training):
            self.logger("Reload params from " + model_path, self.log_file)
            load_torch_model(self.model, model_path)

        if self.configs.use_text_emb:
            self.model.init_projection(self.bert_model)
            self.model.to(device)
            self.optimiser = torch.optim.Adam(self.model.parameters(), lr=self.configs.learning_rate)
            self.opt_lr_scheduler = torch.optim.lr_scheduler.StepLR(self.optimiser, step_size=20, gamma=0.7)
    ...
    def run_train_epoch(self, epoch, batch_size=16):
        ...
        for i, batch in enumerate(tq):
            ...
            batch['token_type'] = batch['token_type'].long()
            ...
            if self.configs.use_text_emb:
                ...
                intent_emb, _ = self.model.bert(input_ids=input_ids, token_type_ids=token_type_ids)
            ...
            bce_loss, details, cvt_values = self.calculate_loss(model_outputs, target_items, raw_data, mode='train')
    ...
    def run_test(self, epoch, batch_size=16, full_batch=False):
        ...
        for i, batch in enumerate(data_loader):
            ...
            input_items = batch['lon'], batch['lat'], batch['alt'], batch['spdx'], batch['spdy'], batch['spdz']
