...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer("mask", torch.tril(torch.ones(config.inp_seq_len + config.horizon, config.inp_seq_len + config.horizon))
...
class FlightBERT_PP(nn.Module):
    def __init__(self, config):
        ...
        self.max_seqlen = config.inp_seq_len
    ...
    def forward(self, lon_inp, lat_inp, alt_inp, spdx_inp, spdy_inp, spdz_inp, dec_inputs, text_emb):
        ...
        if self.config.use_text_emb:
            context_embed = self.multi_modal_fusion(context_embed, text_emb)
    ...
    def multi_modal_fusion(self, context_embed, text_embed,):
        context = torch.cat([context_embed, text_embed], dim=1)
        context = self.proj(context)
        return context

    def init_projection(self, bert_model):
        self.bert = bert_model
        self.proj = nn.Sequential(nn.Linear(self.config.n_embd + self.config.text_embed, 1024),
                                  nn.GELU(),
                                  nn.Linear(1024, self.config.n_embd))
